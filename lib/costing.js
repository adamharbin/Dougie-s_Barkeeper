...
// Traffic-light stock status vs. par level. "not_counted"/"no_par" are
// distinct neutral states — never guess a color with missing data.
export function stockLevelStatus(item) {
  if (item.on_hand_qty == null || item.on_hand_qty === "") {
    return { status: "not_counted", label: "Not counted" };
  }
  const par = Number(item.par_level);
  if (!item.par_level && item.par_level !== 0) return { status: "no_par", label: "No par set" };
  if (par <= 0) return { status: "no_par", label: "No par set" };
  const ratio = Number(item.on_hand_qty) / par;
  if (ratio >= 1) return { status: "stocked", label: "Stocked" };
  if (ratio >= 0.5) return { status: "low", label: "Getting Low" };
  return { status: "critical", label: "Below Par" };
}
...
// null/0 batch_yield_qty = "not a batch recipe" -> divisor of 1 (no change).
// Set = the ingredient list totals a whole batch that makes this many
// servings; food/labor/prime cost divide by it to get a per-serving cost.
function batchDivisor(recipe) {
  const q = Number(recipe.batch_yield_qty || 0);
  return q > 0 ? q : 1;
}
...
export function recipeIngredientCost(recipe, items, prices) {
  ...
  return { total: total / batchDivisor(recipe), hasUnpriced };
}

// labor_cost = (labor_minutes / 60) * hourly rate for the recipe's category,
// divided by batch yield (see batchDivisor). No labor_minutes set => $0
// labor, never estimated.
export function recipeLaborCost(recipe, settings) {
  ...
  return ((minutes / 60) * rate) / batchDivisor(recipe);
}
