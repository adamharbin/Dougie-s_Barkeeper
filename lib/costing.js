...
// converted from the purchase-unit weighted average via units_per_purchase_unit.
// Falls back to 1:1 (same as weightedAvgCost) when an item hasn't set this up —
// existing items/recipes keep working unchanged until someone configures it.
export function costPerRecipeUnit(item, prices) {
  const cost = weightedAvgCost(item.id, prices);
  if (cost == null) return null;
  const factor = Number(item.units_per_purchase_unit || 1);
