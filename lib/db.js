...
export async function insertItem(item) {
  const { data, error } = await getSupabase()
    ...
    .insert({
      ...
      units_per_purchase_unit: numOrNull(item.units_per_purchase_unit) ?? 1,
    })
    .select()
    .single();
  if (error) throw error;
...
export async function insertPrice(price) {
  const { error } = await getSupabase().from("inventory_prices").insert({
    ...
    case_quantity: numOrNull(price.case_quantity),
    units_per_case: numOrNull(price.units_per_case) ?? 1,
