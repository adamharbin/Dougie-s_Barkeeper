...
export async function loadAll() {
  ...
  const [itemsRes, pricesRes, vendorsRes, recipesRes, ingredientsRes, stepsRes, settingsRes] = await Promise.all([
    ...
    sb.from("recipe_steps").select("*").order("step_number", { ascending: true }),
    sb.from("goal_settings").select("*").eq("id", 1).maybeSingle(),
  ]);
  for (const res of [itemsRes, pricesRes, vendorsRes, recipesRes, ingredientsRes, stepsRes, settingsRes]) {
  ...
  const recipes = (recipesRes.data || []).map((r) => ({
    ...
    steps: (stepsRes.data || []).filter((s) => s.recipe_id === r.id),
  }));
...
export async function insertRecipe(recipe) {
  const { data, error } = await getSupabase()
    ...
    .insert({
      ...
      batch_yield_qty: numOrNull(recipe.batch_yield_qty),
      prep_notes: recipe.prep_notes || "",
      menu_category: recipe.menu_category || null,
    })
    .select()
    .single();
  if (error) throw error;
...
// ---- recipe steps (Prep Method build guide) ----
const STEP_IMAGE_BUCKET = "recipe-step-images";

export async function insertStep({ recipe_id, step_number, title, instructions }) {
  const { data, error } = await getSupabase()
    .from("recipe_steps")
    .insert({ recipe_id, step_number, title: title || "", instructions: instructions || "" })
    .select()
    .single();
  if (error) throw error;
  return data;
...
export async function deleteStep(step) {
  const sb = getSupabase();
  if (step.image_url) {
    const path = stepImagePathFromUrl(step.image_url);
    if (path) await sb.storage.from(STEP_IMAGE_BUCKET).remove([path]);
  }
  const { error } = await sb.from("recipe_steps").delete().eq("id", step.id);
  if (error) throw error;
}

export async function uploadStepImage(recipeId, stepId, file) {
  const sb = getSupabase();
  const ext = (file.name.split(".").pop() || "jpg").toLowerCase();
  const path = `${recipeId}/${stepId}-${Date.now()}.${ext}`;
  const { error: uploadError } = await sb.storage.from(STEP_IMAGE_BUCKET).upload(path, file, { upsert: true });
  if (uploadError) throw uploadError;
  const { data } = sb.storage.from(STEP_IMAGE_BUCKET).getPublicUrl(path);
  return data.publicUrl;
}

function stepImagePathFromUrl(url) {
  const marker = `/object/public/${STEP_IMAGE_BUCKET}/`;
...
// ---- invoice library ----
// Loaded on its own (not part of loadAll()) since it's rarely cross-
// referenced elsewhere and could grow large over time — same pattern as
// loadInventoryCounts above.
...
export async function uploadInvoiceFile(file) {
  ...
  const { error } = await sb.storage.from(INVOICE_BUCKET).upload(path, file);
  if (error) throw error;
  return path;
}

// Invoices live in a PRIVATE bucket — always fetch a fresh signed URL to
// view/download one, never store a permanent public link.
export async function getInvoiceSignedUrl(filePath) {
  const { data, error } = await getSupabase().storage.from(INVOICE_BUCKET).createSignedUrl(filePath, 60);
  if (error) throw error;
...
export async function insertInvoice(invoice) {
  ...
  const { error } = await sb.from("invoices").insert({
    ...
    total_amount: numOrNull(invoice.total_amount),
    notes: invoice.notes || "",
...
export async function deleteInvoice(invoice) {
  const sb = getSupabase();
  if (invoice.file_path) await sb.storage.from(INVOICE_BUCKET).remove([invoice.file_path]);
  const { error } = await sb.from("invoices").delete().eq("id", invoice.id);
  if (error) throw error;
}

// ---- waste log ----
// Loaded on its own (not part of loadAll()), same pattern as loadInvoices.
...
export async function insertWasteEntry(entry) {
  const sb = getSupabase();
  const { data: userData } = await sb.auth.getUser();
  const { error } = await sb.from("waste_log").insert({
    item_id: entry.item_id || null,
    waste_date: entry.waste_date || null,
...
export async function updateWasteEntry(id, entry) {
  const { error } = await getSupabase()
    .from("waste_log")
    .update({
      item_id: entry.item_id || null,
...
export async function deleteWasteEntry(id) {
  const { error } = await getSupabase().from("waste_log").delete().eq("id", id);
  if (error) throw error;
}
